...
class WebCalculatorTester:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "healthy":
                        ...
                        print(f"      部署颜色: {data.get('deployment_color')}")
                        return True
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "healthy":
                    ...
                    print(f"[ERROR] 应用状态异常: {data.get('status')}")
            else:
        ...
        try:
            ...
            if response.status_code == 200 and data.get("success") is True:
                ...
                if expected_result is not None and result != expected_result:
                    print(f"[WARN]   预期结果: {expected_result}, 实际结果: {result}")
                self.test_results.append((f"{operation}/{a}&{b}", True))
                return True
            else:
                print(f"[ERROR] ✗ {operation}/{a}&{b} 失败 (状态码: {response.status_code})")
    ...
    def test_error_handling(self):
        ...
        for operation, a, b in error_tests:
            ...
            try:
                ...
                if response.status_code >= 400:
                    print(f"[INFO] ✓ {operation}/{a}&{b} 错误处理正确 (状态码: {response.status_code})")
                    passed += 1
                else:
                    print(f"[ERROR] ✗ {operation}/{a}&{b} 预期错误但得到状态码: {response.status_code}")
            except Exception as e:
                print(f"[ERROR] ✗ {operation}/{a}&{b} 异常: {e}")
        ...
        try:
            ...
            if response.status_code == 404:
                print(f"[INFO] ✓ 不存在的端点错误处理正确 (状态码: 404)")
                passed += 1
            else:
                print(f"[ERROR] ✗ 不存在的端点预期404但得到: {response.status_code}")
        except Exception as e:
            print(f"[ERROR] ✗ 不存在的端点测试异常: {e}")
    ...
    def _test_math_operations(self):
        ...
        if passed == len(math_tests):
            print(f"[INFO] 所有数学运算测试通过 ({passed}/{len(math_tests)})")
            self.test_results.append(("数学运算", True))
        else:
            print(f"[ERROR] 数学运算测试失败: {passed}/{len(math_tests)} 通过")
